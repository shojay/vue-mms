<template>
  <transition name="fade" :appear="true">
    <div class="search">
      ...
      <div class="history-container" v-if="!searching">
        <div class="history">
          <div class="title">
            <div class="chunk"></div>
            <span>历史搜索</span>
          ...
          <div class="tags">
            <div v-for="(item, index) in historyWords" :key="index">
              <tag :text="item" @tag="addToSearch"></tag>
            </div>
          </div>
        </div>
      </div>
      <scroll
        class="books-wrapper"
        :pullup="true"
        @scrollEnd="getMoreBook"
        :data="bookList"
        v-if="searching"
      >
        <div class="books-container" v-if="!noneResult">
          <div v-for="item in bookList" :key="item.id">
            <v-book :book="item" @detail="goToBookDetail"></v-book>
          </div>
          <loading v-show="loading" mt="30px"></loading>
        </div>
        <div class="nobook-container" v-else>
          <svg
            t="1591703747759"
            class="icon"
      ...
      <loading v-show="loadingCenter && searching" mt="200px"></loading>
    </div>
  </transition>
</template>
...
export default {
  ...
  created() {
    this.historyWords = !localStorage.getItem("history")
      ? []
  ...
  methods: {
    ...
    search() {
      ...
      // 搜索书籍
      bookModel.search(this.curStart, this.searchWord).then((res) => {
        this._setMoreBooks(res.data.books, res.data.total);
        this.loadingCenter = false;
      });
      // 存入历史记录
      const index = this.historyWords.indexOf(this.searchWord);
      if (index > -1) {
        this.historyWords.splice(index, 1);
      }
      this.historyWords.unshift(this.searchWord);
      localStorage.setItem("history", this.historyWords);
    ...
    _setMoreBooks(books, total) {
      this.bookList = this.bookList.concat(books);
      this.curStart += books.length;
